...
4. 如果子视图没有调用`requestDisallowInterceptTouchEvent`，那么父视图任何时候都可以拦截事件，即使子视图已经消费了ACTION_DOWN，拦截事件后，会将该事件``setAction(ACTION_CANCEL)``，分发给子视图，之后的事件都将分发给父视图，同时将触摸目标置为 null。
